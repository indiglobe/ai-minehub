      ...
      >
        <div className={cn(`flex w-full flex-col gap-4 @md:flex-row @md:items-center @md:justify-between`)}>
          <div className={cn(`flex items-center gap-2`)}>
            <Clock className={cn(`size-4 text-secondary-500`)} />
          ...
          <div className={cn(`flex items-center gap-3`)}>
            <div className={cn(`relative`)}>
              <select
                ...
                defaultValue="active"
              >
                <option value="active" className={cn(`bg-background text-foreground`)}>
                  Active
                </option>
          ...
          <table className={cn(`w-full text-left border-collapse`)}>
            <thead>
              <tr className={cn(`text-foreground/50 text-2.75 uppercase tracking-wider`)}>
                ...
                <th className={cn(`py-3 font-medium min-w-35`)}>Progress</th>
                <th className={cn(`py-3 font-medium`)}>Status</th>
                <th className={cn(`py-3 font-medium`)}>Started</th>
                <th className={cn(`py-3 font-medium text-right`)}>Ends</th>
              </tr>
            </thead>
            <tbody className={cn(`divide-y divide-foreground/10 text-sm`)}>
              {sessions.map((item) => (
                <tr key={item.id} className={cn(`group`)}>
                  ...
                  <td className={cn(`py-4 pr-4 font-semibold text-foreground/90`)}>
                    {item.plan}
                  </td>
                  <td className={cn(`py-4 pr-4 font-semibold text-primary-500`)}>
                    {item.invested}
                  </td>
                  ...
                  <td className={cn(`py-4 pr-4 font-semibold text-foreground/80`)}>
                    {item.daily}
                  </td>
                  <td className={cn(`py-4 pr-4`)}>
                    <div className={cn(`flex flex-col gap-1`)}>
                      <div className={cn(`bg-foreground/10 relative h-1.5 w-full overflow-hidden rounded-full`)}>
                        <div
                          className={cn(`absolute inset-y-0 left-0 bg-blue-500 rounded-full`)}
                          style={{ width: `${item.progress}%` }}
                        />
                      </div>
                      <span className={cn(`text-2.5 text-foreground/50 text-right`)}>
                  ...
                  <td className={cn(`py-4 text-right text-foreground/60 text-xs`)}>
                    {item.ends}
                  </td>
                </tr>
              ))}
