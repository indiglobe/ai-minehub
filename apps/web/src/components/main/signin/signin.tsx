...
import { GoogleSigninButton } from "@repo/ui/signin-buttons";
...
function SigninFormHeading({ ...props }: ComponentProps<"h2">) {
  const { initiator } = useSearch({
    from: "/(without-header-footer)/(guest)/signin/",
  });
  ...
  return (
    <h2 {...props}>
      ...
      {initiator === "chat-with-expert" && (
        <span
          className={cn(
            `text-primary-500 text-xl font-bold sm:text-2xl lg:text-3xl`,
          )}
        >
          <span>To chat with experts </span>
      ...
      {initiator === "choose-account-type" && (
        <span
          className={cn(
            `text-primary-500 text-xl font-bold sm:text-2xl lg:text-3xl`,
          )}
        >
          <span>For account related process </span>
...
function SigninForm({ className, ...props }: ComponentProps<"div">) {
   ...
   */
  const callbackUrlWithHost = new URL(callbackUrl ?? "", env.VITE_WEB_APP_HOST);
